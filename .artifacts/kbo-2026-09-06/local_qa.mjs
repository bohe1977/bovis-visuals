...
const pages=[['integrated','http://127.0.0.1:4173/kbo/'],['players','http://127.0.0.1:4173/kbo-players/'],['archive-integrated','http://127.0.0.1:4173/kbo/2026-09-06/'],['archive-players','http://127.0.0.1:4173/kbo-players/2026-09-06/']];
const output=[];
for(const [kind,url] of pages) for(const width of [390,768,1440]){
  ...
  await page.goto(url,{waitUntil:'networkidle'}); await page.waitForSelector(kind.includes('players')?'[data-player-card]':'[data-game-card]');
  const result=await page.evaluate(()=>({
    overflow:document.documentElement.scrollWidth<=document.documentElement.clientWidth,
    date:document.body.innerText.includes('2026.09.06'), games:document.querySelectorAll('[data-game-card]').length,
    metrics:[...document.querySelectorAll('.metric strong')].map(x=>x.textContent.trim()),
    inactive:[...document.querySelectorAll('.none')].map(x=>x.textContent.trim()), text:document.body.innerText,
    ...
    decisions:Object.fromEntries(['로드리게스','임찬규','정해영','김원중','박정민','박영현'].map(n=>[n,[...document.querySelectorAll('[data-player-card]')].find(c=>c.querySelector('h3')?.textContent.trim()===n)?.querySelector('.state')?.textContent.trim()||null]))
  }));
  await page.screenshot({path:`.artifacts/kbo-2026-09-06/local-${kind}-${width}.png`,fullPage:true}); output.push({kind,width,errors,...result}); await page.close();
}
await browser.close();
for(const r of output){
  if(!r.overflow||!r.date||r.errors.length||!r.numeric) throw Error(JSON.stringify(r));
  if(r.kind.includes('integrated')&&(r.games!==5||r.metrics.join('|')!=='5|66|6|3'||!r.winnerOrder||!r.metaMono||!r.text.includes('경기 결과')))throw Error(JSON.stringify(r));
  if(r.kind.includes('players')&&(r.inactive.filter(x=>x==='등판 없음').length!==5||r.decisions['로드리게스']!=='패'||r.decisions['임찬규']!=='패'||r.decisions['정해영']!=='홀드'||r.decisions['김원중']!==null||r.decisions['박정민']!==null||r.decisions['박영현']!==null))throw Error(JSON.stringify(r));
}
