...
def test_renderer_embeds_both_modes_as_data_for_synchronized_client_rendering(tmp_path):
    ...
    assert ".menu { padding:7px 9px; border-radius:8px; background:#f2f2f2; color:#444; font-size:13px; font-weight:650; white-space:nowrap; }" in html
    assert ".menus { margin:0; gap:7px; flex-wrap:nowrap; justify-content:center; overflow-x:auto; scrollbar-width:none; -webkit-overflow-scrolling:touch; }" in html
    assert ".menus { flex-wrap:nowrap; justify-content:flex-start; overflow-x:auto; scrollbar-width:none; -webkit-overflow-scrolling:touch; }" in html
