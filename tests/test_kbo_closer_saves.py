...
def test_kbo_report_excludes_canceled_games_from_data_and_display():
    ...
    # 2026-08-16 completed final slate; two scheduled games were canceled and excluded.
    assert sum(game["away_score"] + game["home_score"] for game in games) == 17
...
def test_mlb_game_cards_match_kbo_game_content_hierarchy():
    ...
    assert all(not any("흐름을 주도" in point for point in game.get("game_points", [])) for game in final_games)
...
def test_mlb_current_batting_line_and_team_result_are_normalized_from_official_data():
    ...
    if dodgers_game["status"] == "경기 종료":
        ...
        assert not any("흐름을 주도" in point for point in dodgers_game["game_points"])
