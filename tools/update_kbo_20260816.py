...
# No watched pitcher appeared in a completed game; inactive objects deliberately have no extra fields.
pitchers = [
...
verification = {
    ...
    "details": "2026-08-16 KBO 예정 5경기 중 한화-삼성과 NC-롯데는 취소되어 제외했다. 나머지 3경기는 kbo-game FINISHED, KBO 공식 게임센터 상세기록, 네이버 공개 기록 API statusCode=4·박스스코어, 다음 일정 API gameStatus=END·스코어를 대조했다. 관심 투수는 완료된 경기의 공식·네이버 투수 목록에서 모두 미등판으로 확인했다. 김도영의 4타수 2안타·1삼진·시즌 타율 0.296은 KBO 공식 상세기록과 네이버 API에서 대조했다. 다음은 타자 볼넷을 사사구로 표기할 수 있어 해당 필드는 독립 일치값으로 주장하지 않았다.",
    "conflicts": [],
}
