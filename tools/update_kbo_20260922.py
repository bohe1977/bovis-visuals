#!/usr/bin/env python3
"""Collect and reconcile the 2026-09-22 KBO final slate."""
...
DATE, COMPACT = "2026-09-22", "20260922"
...
assert len(finished) == 4 and len([g for g in daum if g["gameStatus"] == "END"]) == 4
...
verification = {"status": "KBO 공식 기준 · kbo-game·네이버·다음 대조", "sources": ["KBO 공식 게임센터 REVIEW/API", "kbo-game", "네이버스포츠 공개 기록 API", "다음스포츠 일정·박스스코어"], "details": f"{DATE} KST 편성 4경기는 kbo-game FINISHED, KBO 공식 게임센터 GetScoreBoardScroll·GetBoxScoreScroll(code=100), 네이버 기록 API(statusCode=4), 다음 일정 API(gameStatus=END)로 종료 상태와 스코어를 대조했다. 종료 4경기 합계는 {total_runs}득점이다. 류현진과 제레미 비슬리의 실제 등판 라인·시즌 승패·ERA는 KBO 공식 투수표와 네이버 행으로 대조했고, 두 선수 모두 공식 선발 행과 각각 패전·승리를 확인했다. 강백호의 당일 타격 라인·시즌 타율은 공식 경기 타자표와 네이버 행, 시즌 OBP·OPS는 KBO 공식 선수 상세기록으로 확인했다. 미등판·미출전 관심 선수는 해당 완료 경기의 KBO 공식·네이버 전체 명단 부재로 계약상 최소 객체로 보존했다. 다음 타자 표의 사사구 표기 범위 차이로 볼넷 독립 일치값은 주장하지 않았다.", "conflicts": []}
...
for rel in ("kbo/index.html", "kbo-players/index.html"):
    ...
    text = re.sub(r"2026\.09\.\d{2}", "2026.09.22", text)
    text = re.sub(r"2026년 9월 \d{1,2}일", "2026년 9월 22일", text)
    path.write_text(text, encoding="utf-8")
(ART / "verification.json").write_text(json.dumps({"report_date": DATE, "games": len(games), "total_runs": total_runs, "source_status": {"kbo_game": "FINISHED x4", "kbo_official": "code=100 x4", "naver": "statusCode=4 x4", "daum": "gameStatus=END x4"}, "watched_pitchers": [p["name"] for p in pitchers if p["appeared"]], "watched_batters": [b["name"] for b in batters if b["appeared"]]}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
