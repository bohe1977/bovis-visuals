...
TEAM_KO={'Los Angeles Dodgers':'LA 다저스','San Francisco Giants':'샌프란시스코','Arizona Diamondbacks':'애리조나','Detroit Tigers':'디트로이트','Boston Red Sox':'보스턴','New York Mets':'뉴욕 메츠','Philadelphia Phillies':'필라델피아','Kansas City Royals':'캔자스시티','Pittsburgh Pirates':'피츠버그','Los Angeles Angels':'LA 에인절스','Chicago White Sox':'시카고 화이트삭스','San Diego Padres':'샌디에이고','Atlanta Braves':'애틀랜타','Milwaukee Brewers':'밀워키','Seattle Mariners':'시애틀'}
...
def main():
    ...
    for name,pid,_ in PLAYER_SPECS[8:]:
      ...
      if team_sports.get(pid) != 1:
        pitchers.append({'name':name,'team':'','mlbam_id':pid,'appeared':False,'minor_league_excluded':True,'status':'메이저 출전 없음','daily_note':'MLB 경기 출전 없음'})
        continue
      apps=[]
      for pk,g in tg.items():
